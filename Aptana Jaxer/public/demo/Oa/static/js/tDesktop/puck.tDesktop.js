...
define('tDesktop/tDesktop.IM', function (require, exports, module) {
    ...
    var Comet = {
        init: function (c) {
            ...
            var self = this;
            $.ajax({
                async: true, url: 'ipanel/user/user_count.php', dataType: 'text', success: function (data) {
                    try {
                        eval(data);
                    }
                    catch (ex) {
    ...
    var Chat = {
        init: function (c) {
            ...
            self.tDesktop.EventManager.on('msg:read', function (sms) {
                var online = null;
                $.ajax({
                    type: 'GET', url: '/general/userinfo.php', data: {'UID': sms.from_id}, success: function (d) {
            ...
            var self = this, $list = $('#smsbox_msg_container'), avatar = $("#loginavatar").attr("src");
            ...
            self.uploader.on('fileQueued', function (file) {
                var date = new Date(), time = date.toTimeString().substr(0, 5), filesize = WebUploader.Base.formatSize(file.size);
            ...
            self.uploader.on('uploadProgress', function (file, percentage) {
                var $li = $('#' + file.id), $percent = $li.find('.progress .bar');
                if (!$percent.length) {
            ...
            self.uploader.on('uploadSuccess', function (file) {
                ...
                $.ajax({
                    ...
                    data: {uid: uid, userid: userid, fileid: fileid, filename: file.name, flag: 0},
                    success: function (data) {
                    }
                });
                setTimeout(function () {
            ...
            var msg = $('#smsbox_textarea').val(), $button = $("#smsbox_send_msg");
            if (!msg) {
                return;
            }
            ...
            users: [], curPage: 1, maxPageLimit: 10, itemsPerPage: 6, $el: $('#moreHistoryMsg'), init: function () {
            }, loadMore: function (currentUid) {
                var self = this;
                var uid = currentUid;
                if (self.users[uid] === undefined) {
                    self.users[uid] = {curPage: 1, lastId: '', historyMessage: []};
                }
    ...
    var RecentList = {
        init: function (c) {
            ...
            var avatar = "";
            $.ajax({
                type: 'GET', url: '/general/userinfo.php', data: {'UID': uid}, success: function (d) {
        ...
        init: function (c) {
            ...
            $('#search').typeahead({
                source: function (query, process) {
                    $.ajax({
                        url: 'search.php', data: {term: query}, async: true, type: 'get', success: function (d) {
                            d = JSON.parse(d);
                            userlist = d;
            ...
            $.ajax({
                ...
                data: {fileid: fileid, filename: file.name},
                async: true,
                type: 'get',
                success: function (d) {
    ...
    exports.IM = {
        ...
        }, Comet: Comet, Chat: Chat, RecentList: RecentList, MultiSend: MultiSend, Org: Org
    }
});
    ...
    var Layout = Backbone.View.extend({
        ...
        initRegPanel: function () {
            if ($('#hero_bar').length > 0) {
                $('#east,#west,#center').css('bottom', '31px');
            } else {
...
define('tDesktop/tDesktop.Menu', function (require, exports, module) {
    ...
    var secondMenuList = Backbone.Collection.extend({
        model: secondMenu, url: '', sync: function () {
        }
    });
    ...
    var MenuView = Backbone.View.extend({
        el: '.west-body', initialize: function () {
            var self = this;
...
define('tDesktop/tDesktop.Nocbox', function (require, exports, module) {
    ...
    var Nocbox = {
        init: function (c) {
            ...
            $.ajax({
                type: 'POST', url: 'status_bar/sms_submit.php', data: {'SMS_ID': recvIdStr}, success: function (data) {
...
define('tDesktop/tDesktop.Notification', function (require, exports, module) {
    ...
    var Msg = Backbone.Model.extend({
        ...
        addItem: function (item) {
            ...
            if ($.inArray(item.key, allKeys) === -1) {
                ...
                this.set({title: item.title, mid: item.mid, msg: item.msg, num: this.size(), url: item.url});
            }
        },
        removeLastItem: function () {
            ...
            if (last) {
                this.set({title: last.title, mid: last.mid, msg: last.msg, num: this.size(), url: last.url});
            } else {
                this.destroy();
            }
        },
    ...
    var MsgList = Backbone.Collection.extend({
        model: Msg, url: 'index.php', addItem: function (item) {
    ...
    var ListView = Backbone.View.extend({
        el: $('.msg-list'), initialize: function () {
            ...
            $('.msg-list').animate({right: -300, opacity: 0}, {
    ...
    function _calculateItemPosition() {
        var totalHeight = $('.center').height() + parseInt(jQuery('.center').css('bottom')) - 75, singleHeight = $('.msg-list .msg-wrapper').outerHeight(true) || 70, itemCount = Math.floor(totalHeight / singleHeight), elBottom = totalHeight - itemCount * singleHeight, el = $('.msg-list');
...
define('tDesktop/tDesktop.Search', function (require, exports, module) {
    ...
    var searchModule = {$el: $('.search-container'), paginators: {}, templates: []};
    ...
    searchModule.initBind = function () {
        ...
        $(".search-container").delegate(".user-body-follow", "click", function () {
            var $this = $(this), uid = $this.parents("li").attr("u_id"), userid = $this.parents("li").attr("userid");
        ...
        $('#search-input').one('keydown', function (e) {
            $('.search-box').css({'margin-top': '15px', 'margin-left': '0px'});
        });
        if (tDesktop.isTouchDevice()) {
        ...
        $('.search-results-workflow').bind('ajaxDataReady', function () {
            $.each($('.search-results-workflow>li'), function (index, item) {
                ...
                $(item).find('a').attr({'href': newUrl, 'target': '_blank'});
            })
        });
    ...
    function search(keyword, searchType, curPage, pageLimit) {
        ...
        if (searchType == 'menu') {
            ...
            for (var menuId in func_array) {
                var func = func_array[menuId], func_id = menuId.substr(1);
        ...
        $.ajax({
            ...
            data: {curPage: _curPage, pageLimit: _pageLimit, type: searchType, keyword: keyword},
            dataType: "json",
            success: function (data) {
    ...
    var Tabs = {
        ...
        createTab: function (id, name, code, open_window) {
            ...
            if (id >= 10000 && id <= 14999)
                url = '/fis/' + code; else if (id >= 15000 && id <= 15499)
                url = '/hr/' + code; else if (id >= 650 && id <= 1000 || code.length > 4 && code.substr(code.length - 4).toLowerCase() == ".jsp")
                url = '/app/' + code; else
                url = '/general/' + code;
...
define('tDesktop/tDesktop.Today', function (require, exports, module) {
    ...
    var Reminder = Backbone.View.extend({
        initialize: function () {
            ...
            $.ajax({
                url: '/general/task_center/getReminder.php', data: "", async: true, type: 'get', success: function (d) {
                    if (d.length > 0) {
                        $.each(d, function (k, v) {
